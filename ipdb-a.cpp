...
IpdbCT *
Ipdb::Enc(GT M77, Big *X, int len, int start, Big s1, Big s2, Big s3, Big s4)
{

	IpdbCT *Ct;
	Ct=new IpdbCT(len);

	printf("start of Enc (len=%d start=%d)\n",len,start);
	Ct->A=pfc->mult(g1,s2);
...
Ipdb::DecRow(EncryptedRow Ct, QueryKey QQ, int Cell)
{
	...
	G1 **right=new G1*[4*(m+1)+4*3+2];
	G2 **left= new G2*[4*(m+1)+4*3+2];

	printf("First loop\n");
	left[0]=&(QQ.Key[0]->KA); right[0]=&(Ct.ek[0]->A); 
	left[1]=&(QQ.Key[0]->KB); right[1]=&(Ct.ek[0]->B); 
	int j=2;
	for(int i=0;i<m+1;i++) {
	...
	GT tmpT1=pfc->multi_pairing(4*(m+1)+2,left,right);

	printf("Second loop\n");
	left[0]=&(QQ.Key[1]->KA); right[0]=&(Ct.ek[Cell+1]->A); 
	left[1]=&(QQ.Key[1]->KB); right[1]=&(Ct.ek[Cell+1]->B); 
	j=2;
	for(int i=0;i<3;i++) {
		printf("i=%d\n",i);
	...
	GT tmpT2=pfc->multi_pairing(14,left,right);

	//GT tmpT=Ct.ek[Cell+1]->C/pfc->multi_pairing(4*(m+1)+14,left,right);
...
Ipdb::QueryKeyGen(char **CQuery, int Cell){
	...
	KeyAttribute[0]=-1; KeyAttribute[1]=(Big) Cell; KeyAttribute[2]=modmult(-1,KeyAttribute[2*n+2],order);
