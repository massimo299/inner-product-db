...
#define n 3             // number of attributes
...
// Encrypt
	...
	G1 tmpG1; pfc.random(tmpG1);
	GT pt=pfc.pairing(g2,tmpG1);
	GT Mpt=pt*MME;
...
// Decrypt
	...
	j=2;
	for (i=0;i<n;i++){
		left[j]=&K1[i]; right[j]=&C1[i]; j++;
		left[j]=&K2[i]; right[j]=&C2[i]; j++;
		left[j]=&K3[i]; right[j]=&C3[i]; j++;
		left[j]=&K4[i]; right[j]=&C4[i]; j++;
	}
